...
class ValidationService {
  ...
  ProcessingReport validate(MetadataRecord metadataRecord) {
    ...
    if (!results && !results.size()) {
      throw new IllegalArgumentException('Record references non-existent schema: ' + schemaId)
    } //controller advice catches it and returns 400
  ...
  Map fetchDefinitions(Map schemaJson) {
    ...
    (refs - defs).collectEntries(definitions) {
      if (!(it in definitions)) {
        ...
        if (!schemaList && !schemaList.size()) {
          throw new IllegalArgumentException('Schema references non-existent object: ' + it)
        } //controller advice catches it and returns 400
